 ...
 */
 ...
 public class StaffUI extends JPanel implements BookingObserver 
 {
   ...
   public StaffUI(JFrame f)
   {
       ...
       width = LEFT_MARGIN + (SLOTS * COL_WIDTH);
       height = TOP_MARGIN + tableNumbers.size()*ROW_HEIGHT + BOTTOM_MARGIN;
   ...
   // from BookingObserver Interface
   public void update()
   {
       repaint() ;
   }
  ...
  //  System.out.println("JPanel dimension is " + canvasSize);
  ...
  public void updateReservation(){
    if(bs.getSelectedBooking() != null){
      if(bs.getSelectedBooking() instanceof Reservation){
        UpdateReservationDialog d = new UpdateReservationDialog(
          parentFrame,
          "Update reservation details") ;
        d.setVisible(true) ;

        if(d.confirmed){
          bs.updateReservation(d.getCovers(),
          bs.getCurrentDate(),
          d.getTime(),
          d.getTableNumber(),
          d.getCustomerName(),
        ...
        bs.observerMessage("Only reservations can be updated", false);  
      } 
    }else{
      bs.observerMessage("please select a reservation", false);
    }
  }
}
